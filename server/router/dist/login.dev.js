...
var _require2 = require('../models/Dancer'),
    Dancer = _require2.Dancer; // auth 라는 middleware 을 가져온다 ( 인증처리 )


var _require3 = require('../middleware/auth'),
    auth = _require3.auth;

var jwt = require('jsonwebtoken'); // mailgun, email account acivation
...
var DOMAIN = 'sandbox7de21197a67843cba3b19ca2e899ec14.mailgun.org';
var mg = mailgun({
  apiKey: "07424c709ace08ce574c0895c854437e-913a5827-c6a0af4b",
  domain: DOMAIN
}); // 로그인 기능 : login
...
router.post('/api/users/forgetPassword', function (req, res) {
  ...
  User.findOne({
    ...
    var data = {
      ...
      html: "\n                <h2>Please click on given link to reset your password</h2>\n                <a href = \"http://localhost:4000/api/users/resetPassword/".concat(token, "\">http://localhost:4000/api/users/resetPassword/").concat(token, "</a>\n            ")
    ...
    return user.updateOne({
      ...
      if (err) {
        ...
        return res.status(400).json({
          ...
          'result': 'reset password link error, contact Us'
        });
      } else {
        // send email 
...
router.post('/api/users/resetPassword/:token', function (req, res) {
  ...
  var _req$body = req.body,
      ...
      newPass = _req$body.newPass; // 만약 link가 존재한다면
    ...
    jwt.verify(resetLink, "accountactivatekey123", function (error, decodedData) {
      ...
      if (error) {
        ...
        return res.status(401).json({
          ...
          "result": "Incorrect token or it is expired"
        });
      ...
      User.findOne({
        ...
        if (error || !user) {
          ...
          setTimeout(function () {
            return res.status(400).json({
              ...
              "result": "User with this token does not exist"
            });
          }, 2000);
        ...
        user = _.extend(user, obj); // update 이후 save it into DB
