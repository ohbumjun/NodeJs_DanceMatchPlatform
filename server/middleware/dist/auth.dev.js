...
var auth = function auth(req, res, next) {
  ...
  var token = req.cookies.x_auth; // token을 decode 시켜서, role이 user인지 dancer 인지를 파악한다
  ...
  jwt.verify(token, "accountactivatekey123", function (err, decodedToken) {
    ...
    User.findByToken(token, function (err, user) {
      if (err) throw err; // user 가 없다면 클라이언트에 다음과 같은 메시지를 send 한다  
      ...
      req.token = token;
      req.user = user;
      console.log('auth user id', req.user._id); // response.setHeader("Content-Type", "text/html");
